...
hooksecurefunc("ToggleDropDownMenu", function(level)
	...
	local menuTop = listFrame:GetTop()
	local menuBottom = listFrame:GetBottom()

	-- Check if menu is offscreen vertically
	local offTop = menuTop and menuTop > screenHeight
	local offBottom = menuBottom and menuBottom < 0

	if not offTop and not offBottom then
		return
	end

	-- Get current anchor
	local point, anchorFrame, relativePoint, xOff = listFrame:GetPoint(1)
	...
	-- Calculate Y offset to keep menu on screen
	local menuHeight = listFrame:GetHeight()
	local anchorTop = anchorFrame:GetTop()
	local projectedBottom = anchorTop - menuHeight
